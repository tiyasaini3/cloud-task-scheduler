...
def create_task(payload: schemas.TaskCreate, db: Session = Depends(get_db)):
    ...
    for minutes in offsets:
        ...
        if remind_at > datetime.now(timezone.utc):
            ok = enqueue_reminder(str(task.id), task.title, task.owner_id, remind_at, deadline)
            if ok:
                queued += 1
    ...
    storage.write_audit_log("task_created", str(task.id), task.owner_id, {
        "title": task.title, "deadline": str(task.deadline), "reminders_queued": queued,
    })
...
def list_tasks(
    ...
    total = q.count()
    tasks = q.order_by(models.Task.deadline.asc()).offset((page - 1) * page_size).limit(page_size).all()
...
def delete_task(task_id: UUID, db: Session = Depends(get_db)):
    task = _get_or_404(task_id, db)
    storage.write_audit_log("task_deleted", str(task.id), task.owner_id, {"title": task.title})
    db.delete(task)
    db.commit()
    cache.delete_job_state(str(task_id))
    cache.invalidate_task_cache(str(task_id))
    return schemas.MessageResponse(message="Task deleted.", task_id=str(task_id))
...
def get_reminders(task_id: UUID, db: Session = Depends(get_db)):
    ...
    ).order_by(models.ReminderLog.scheduled_for.asc()).all()
